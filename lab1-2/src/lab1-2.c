...
void drawTree(){
		...
		struct Queue* queueX = createQueue(1000);
		struct Queue* queueY = createQueue(1000);
		enqueue(queueX, x0);
		enqueue(queueX, x1);
		enqueue(queueY, y0);
		enqueue(queueY, y1);
		 ...
		 double lamda = 0.8;
		 double randAngle = 10 + rand()%20;
		double alpha =  randAngle/ 180.0 * 3.14;
		...
		uint32_t randColor = colors[rand()%5];
		 for (int i = 0; i < 10 ; i++){
			 ...
			 drawLine(x0,y0,x1,y1,randColor);

			 int x2 = x1 + lamda * (x1 - x0);
			 int y2 = y1 + lamda * (y1 - y0);
			 if (y1 == y0 && x0 == x1) {
				 break;
			 }
			 	 ...
			 	 double t11 = cosA;
			 	 double t12 = -sinA;
			 	 double t13 = -x1 * cosA + y1 * sinA + x1;
			 	 double t21 = sinA;
			 	 double t22 = cosA;
			 	 double t23 = -x1*sinA - y1 * cosA + y1;


			 double x3 = t11 *x2 + t12 * y2 + t13;
			 double y3 = t21 *x2 + t22 * y2 + t23;
			 t13 = -x1 * cosA - y1 * sinA + x1;
			 t23 = x1 * sinA - y1 * cosA + y1;
			 double x4 = t11 * x2 - t12* y2 + t13;
			 double y4 = -t21 * x2 + t22 * y2 + t23;
